...
This file records only the validation performed while assembling the packet. It is not the final empirical pilot report and must not replace `reports/FINAL_HANDOFF.md`, which Claude Cowork is instructed to create after rerunning the repository in its own environment.
...
Claude Cowork must rerun all checks, attempt live metadata harvesting using only credentials already present, inspect figures, make any repairs, produce the final run log and handoff, initialize git, and commit the final repository. The exact instructions are in `COWORK_TASK.md` and `.claude/CLAUDE.md`.
